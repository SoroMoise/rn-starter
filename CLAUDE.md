...
**A declared permission is a promise on the store listing.** `android.blockedPermissions` (`app.config.js`) removes from the whole manifest merge what nothing here uses: `SYSTEM_ALERT_WINDOW`, `READ_EXTERNAL_STORAGE` and `WRITE_EXTERNAL_STORAGE`, which the Expo template declares and `expo-file-system` declares again — dropping them from the template's manifest alone would not remove them. The removal lands in `src/main`, so it holds for the release; the debug build keeps `SYSTEM_ALERT_WINDOW`, which `src/debug` declares for the dev menu. A blocked permission fails at runtime, never at build time, so a feature that needs one takes it off the list in the same change — a media picker needs `READ_EXTERNAL_STORAGE` up to Android 12. `POST_NOTIFICATIONS` and `RECEIVE_BOOT_COMPLETED` never go on it (Notifications).
...
- **`@revenuecat/purchases-js-hybrid-mappings` resolves to an empty module on Android and iOS** (`metro.config.js`). react-native-purchases requires its browser implementation at import, whatever the platform, and runs it only without the native module (Expo Go, web): ~740 KB, the largest package in the bundle, that no native build runs. The stub stays gated on `platform !== 'web'`, where that SDK is the only purchase path.
