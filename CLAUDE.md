...
- **Contextual paywall** — `contextualPaywallService` triggers the paywall at a value moment once the generic action count (`engagementStorage.getActionCount()`) reaches `minActions`. The session count only holds it back during the first session: coming back proves nothing about value drawn from the app, and as an OR beside the action count it always fired first. `power_action` / `after_n_actions` / `rewarded_ad_dismissed` name the source; none fires at launch — a cold start is not a value moment.
