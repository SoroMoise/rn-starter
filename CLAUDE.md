...
- **An action is counted first, then offered to one promo, in a fixed order.** `recordAction()` increments the lifetime counter whatever follows, then offers the moment to the contextual paywall, then to the interstitial, then to the rating prompt; the first to take it ends the chain, and all three draw on the session's single interruption. The paywall comes first on purpose — it is the surface that earns — so do not flip the order to show the ad sooner. A moment that must not be interrupted still counts: `recordAction({ allowPromos: false })` for the user's first success, which decides whether they come back tomorrow, and for an abandoned or failed action, which has nothing to celebrate.
